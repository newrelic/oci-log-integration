...
// be reconstructed as valid JSON here -- as the CloudEvent directly (type/data/source/oracle at
// the top level), matching what Service Connector Hub actually delivers. Every field this
// package actually reads is preserved verbatim from the original; irrelevant nested blocks
// (identity/request/response/etc.) are collapsed to {} since nothing under test reads into them.
// Samples whose type has no rule in rules.go at all -- Vault, Compute, Events ListRules, Queue,
// and Streaming -- were removed (see extract_test.go's own coverage of "unmatched" behavior in
// general, and rules.go's own doc comment on why that's deliberate). The Bastion samples are kept
// specifically to prove Bastion is unmatched too, now that its rule has been removed for the same
// "no confirmed entity-definitions rule" reason.
