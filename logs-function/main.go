...
func handleFunction(ctx context.Context, in io.Reader, out io.Writer) {
	...
	runStart := time.Now()
	defer func() {
		rec.Summary(metrics.TierAdvanced, "forwarder.run.duration", time.Since(runStart).Seconds(), nil)
	}()
	...
	if err != nil {
		...
		rec.Count(metrics.TierAdvanced, "forwarder.client.init.errors", 1, map[string]interface{}{"error_class": util.NRClientErrorClass(err)})
